...
class LighthouseWrapper:
    def get_accessibility(self, url: str) -> dict[str, Any]:
        try:
            result = self.get_categories(
                url=url, categories=[LighthouseCategories.ACCESSIBILITY]
            )
    ...
    def get_best_practices(self, url: str) -> dict[str, Any]:
        try:
            result = self.get_categories(
                url=url, categories=[LighthouseCategories.BEST_PRACTICES]
            )
        except LighthouseError as e:
    ...
    def get_categories(
        self, url: str, categories: list[LighthouseCategories]
    ) -> dict[str, Any]:
        try:
