...
class BaseTest(unittest.TestCase):
    ...
    def mock_response(self, url="http://example.com/test", depth=1, status=200, text="mocked_content"):
        response = MagicMock()
        response.url = url
...
class TestCustomCloseSpider(BaseTest):
    ...
    def test_page_count_with_robots_txt(self, mocked_page_count):
        ...
        middleware.page_count(self.mock_response(), self.mock_request(url="http://example.com/robots.txt"), self.mock_spider())
        mocked_page_count.assert_not_called()

    @patch("crawler.middlewares.CloseSpider.page_count")
    def test_page_count_without_robots_txt(self, mocked_page_count):
        middleware = CustomCloseSpider(MagicMock())
        middleware.page_count(self.mock_response(), self.mock_request(), self.mock_spider())
...
class TestCustomHeadersMiddleware(unittest.TestCase):
    ...
    def test_from_crawler(self):
        ...
        expected_headers = [("Header1", "Value1"), ("Header3", "Value3"), ("HeaderA", "ValueA")]
...
class TestHtmlStorageMiddleware(BaseTest):
    ...
    @patch.dict("os.environ", {"LOCAL_FILES_PATH": "/tmp", "HTML_FOLDER_NAME": "html"})
    def test_process_response(self, mock_mkdir, mock_write_text):
        mock_response = self.mock_response()
        result = self.middleware.process_response(self.mock_request(), mock_response, self.mock_spider())
    ...
    def test_robots_txt_skip(self, mock_mkdir, mock_write_text):
        ...
        result = self.middleware.process_response(self.mock_request(url="http://example.com/robots.txt"),
                                                  mock_response,
                                                  self.mock_spider())
    ...
    def test_page_limit_reached(self):
        ...
        with self.assertRaises(Exception) as context:
            self.middleware.process_response(None, self.mock_response(), self.mock_spider())
...
class TestMetadataMiddleware(BaseTest):
    ...
    def test_process_spider_output(self):
        ...
        modified_result = list(self.middleware.process_spider_output(self.mock_response(), result, mock_spider))
        mock_spider.crawl_process.save_url_for_metadata.assert_called_once_with(self.mock_response().url, self.mock_response().meta["depth"])
